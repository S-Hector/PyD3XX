...
if Status != PyD3XX.FT_OK:
    ...
    exit()
# PipeIndex is 0, which is channel 1 OUT/write endpoint.
# Following queue will automatically queue up given write requests.
Status, WriteQueue = PyD3XX.Queue.CreateQueue(Device, WritePipe, 1, QUEUE_SIZE, False)
if(Status != PyD3XX.FT_OK):
...
Status, ReadQueue = PyD3XX.Queue.CreateQueue(Device, ReadPipe, 1, QUEUE_SIZE, False)
if(Status != PyD3XX.FT_OK):
    print(PyD3XX.FT_STATUS_STR[Status] + " | FAILED TO CREATE READ QUEUE!")
    exit()
print("Successfully made read queue!")

ReadBuffer = PyD3XX.FT_Buffer.from_int(0)
ReadValue = 0
while True:
    WriteBuffer = PyD3XX.FT_Buffer.from_int(ReadValue + 1)
    Status = PyD3XX.Queue.WriteQueue(WriteQueue, WriteBuffer, True) # Send write request.
    if(Status != PyD3XX.FT_OK):
        print(PyD3XX.FT_STATUS_STR[Status] + " | FAILED TO WRITE TO QUEUE!")
        exit()
    Status = PyD3XX.Queue.GetWriteStatus(WriteQueue, True) # Get status of the oldest write request
    if(Status != PyD3XX.FT_OK):
        print(PyD3XX.FT_STATUS_STR[Status] + " | LAST WRITE REQUEST FAILED!")
        exit()
    print("Data Out = " + str(ReadValue + 1))
    Status, ReadBuffer, BytesTransferred = PyD3XX.Queue.ReadQueue(ReadQueue, True)
    if(Status != PyD3XX.FT_OK) and (Status != PyD3XX.FT_IO_INCOMPLETE) and (Status != PyD3XX.FT_IO_PENDING) and (Status != PyD3XX.FT_NO_MORE_ITEMS):
        ...
        exit()
    ReadValue = int.from_bytes(ReadBuffer.Value(), "little")
