...
CUSTOM_VID = int("0xDEAD",16)
CUSTOM_PID = int("0xBEEF",16)

# User access lasts until device is unplugged/reset/power cycled.
if(PyD3XX.Platform == "linux"):
    Status = PyD3XX.GrantUserAccess(VID = CUSTOM_VID, PID = CUSTOM_PID)
    if(Status != PyD3XX.FT_OK):
        print(PyD3XX.FT_STATUS_STR[Status] + " | FAILED TO GRANT USER ACCESS: ABORTING")
        exit()
    print("Successfully granted user access!")

Status = PyD3XX.FT_SetVIDPID(CUSTOM_VID, CUSTOM_PID) # Add custom VID:PID combination for device detection.
if Status != PyD3XX.FT_OK:
    print(PyD3XX.FT_STATUS_STR[Status] + " | FAILED TO ADD VID:PID COMBINATION: ABORTING")
    exit()
...
if Status != PyD3XX.FT_OK:
    print(PyD3XX.FT_STATUS_STR[Status] + " | FAILED TO GET VID:PID COMBINATION: ABORTING")
    exit()
